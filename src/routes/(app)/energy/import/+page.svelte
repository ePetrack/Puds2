...
<svelte:head>
	<title>Import Readings - Energy Management Platform</title>
</svelte:head>
...
<div class="mx-auto max-w-3xl space-y-6">
	<div>
		...
		<h1 class="text-3xl font-bold text-gray-900 dark:text-white">Import Readings from CSV</h1>
	</div>

	<div class="card space-y-4 p-6 text-sm text-gray-600 dark:text-gray-400">
		<p>
			Rows are matched to meters by
			<code class="rounded bg-gray-100 px-1 py-0.5 text-xs dark:bg-gray-800">meter_number</code>.
			One reading per meter per date; duplicates are skipped and reported.
		</p>
		<p>
			Required columns: <strong>meter_number, reading_date, usage</strong>. Optional: demand_kw,
			cost, reading_type (actual/estimated), notes. Dates are YYYY-MM-DD.
		</p>
		<a
			href="/templates/energy-readings-template.csv"
			download
	...
	>
		...
		<div class="flex justify-end gap-3">
			...
			<button type="submit" disabled={submitting} class="btn btn-primary">
				{submitting ? 'Importing...' : 'Import Readings'}
			</button>
		</div>
	</form>
	...
	{#if form?.result}
		<div class="card space-y-4 p-6">
			...
			{#if form.result.imported > 0}
				<a href="/energy" class="btn btn-primary inline-block">View Readings</a>
			{/if}
		</div>
	{/if}
</div>
