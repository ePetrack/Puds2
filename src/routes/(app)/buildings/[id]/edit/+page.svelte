<script lang="ts">
  import { pb, type Client, type Building } from '$lib/pocketbase';
  ...
  let loadingBuilding = $state(true);
  ...
  onMount(async () => {
    await Promise.all([loadClients(), loadBuilding()]);
  });
  ...
  async function loadBuilding() {
    try {
      ...
      // Populate form
      formData.client = building.client;
      formData.name = building.name;
      formData.square_footage = building.square_footage?.toString() || '';
      formData.building_type = building.building_type || '';
      formData.year_built = building.year_built?.toString() || '';
      formData.address = building.address || '';
      formData.floors = building.floors?.toString() || '';
      formData.occupancy = building.occupancy?.toString() || '';
      formData.notes = building.notes || '';
    } catch (error: any) {
  ...
  async function handleSubmit(e: Event) {
    ...
    try {
      ...
      await pb!.collection('buildings').update(buildingId, data);
      toast.success('Building updated successfully');
      goto(`/buildings/${buildingId}`);
    } catch (error: any) {
...
<svelte:head>
  <title>Edit {building?.name || 'Building'} - Energy Management Platform</title>
</svelte:head>
...
<div class="max-w-3xl mx-auto space-y-6">
  ...
  <div>
    <div class="flex items-center gap-2 mb-2">
      <a href="/buildings/{buildingId}" class="text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-100">
        ← Back to Building
      </a>
    </div>
