<script lang="ts">
  ...
  async function handleSubmit(e: Event) {
    ...
    try {
      ...
      // Update client
      await pb.collection('clients').update(clientId, validated);
...
<svelte:head>
  <title>Edit {formData.name || 'Client'} - Energy Management Platform</title>
</svelte:head>
  ...
  {:else}
    ...
    <div>
      <div class="flex items-center gap-2 mb-2">
        <a href="/clients/{clientId}" class="text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-100">
          ← Back
        </a>
      </div>
    ...
    <form onsubmit={handleSubmit} class="card p-6">
      <div class="space-y-6">
        ...
        <div class="flex items-center justify-end gap-3 pt-6 border-t border-gray-200 dark:border-gray-700">
          <a href="/clients/{clientId}" class="btn btn-secondary">
            Cancel
          </a>
